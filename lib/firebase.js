...
if (hasValidConfig) {
  ...
  console.error("❌ Faltan variables de entorno para Firebase. La aplicación funcionará en modo offline/mock.")
}
...
if (!app || !db || !storage) {
  ...
  db = db || {
    ...
    runTransaction: async (fn) => await fn({ get: async () => ({ exists: false, data: () => ({}) }) }),
  }
