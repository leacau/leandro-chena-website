...
import { Clipboard, ExternalLink, Trash2, RefreshCw } from "lucide-react"
...
export default function UrlShortener() {
  ...
  const baseUrl = typeof window !== "undefined" ? `${window.location.origin}/go/` : ""

  // Cargar URLs existentes
  const loadShortUrls = async () => {
    setIsLoadingUrls(true)
    try {
      const urlsQuery = query(collection(db, "shortUrls"), orderBy("createdAt", "desc"))
  ...
  const createShortUrl = async () => {
    ...
    try {
      ...
      // Guardar en Firestore - Asegurarse de usar el campo longUrl
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Crea URLs cortas para compartir fácilmente</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Si lo dejas en blanco, se generará automáticamente</p>
          </div>
        </CardContent>
        <CardFooter>
          <Button onClick={createShortUrl} disabled={isLoading} className="w-full">
            {isLoading ? (
              <>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={loadShortUrls} disabled={isLoadingUrls}>
            <RefreshCw className={`h-4 w-4 mr-2 ${isLoadingUrls ? "animate-spin" : ""}`} />
            Actualizar
          </Button>
        </CardHeader>
        <CardContent>
          {isLoadingUrls ? (
            ...
            <p className="text-center py-6 text-muted-foreground">No hay URLs acortadas todavía</p>
          ) : (
            <div className="space-y-4">
              {shortUrls.map((url) => (
                <div key={url.id} className="flex flex-col space-y-2 p-4 border rounded-lg">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-sm text-muted-foreground">{url.clicks || 0} clics</div>
                  </div>
                  <div className="flex space-x-2">
                    <Button variant="outline" size="sm" onClick={() => copyToClipboard(`${baseUrl}${url.slug}`)}>
                      <Clipboard className="h-4 w-4 mr-2" />
                      Copiar
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => window.open(`${baseUrl}${url.slug}`, "_blank")}>
