...
export default function FirebaseDiagnostics() {
	...
	useEffect(() => {
		...
		if (missingVars.length > 0) {
			setStatus('warning');
			setMessage(
				`Faltan algunas variables de entorno: ${missingVars.join(', ')}`
			);
		} else {
			setStatus('success');
		...
		try {
			...
			if (found) {
				setTestResult({
					status: 'success',
					message:
						'Conexión a Firestore exitosa. Se pudo escribir y leer datos.',
				});
			} else {
				setTestResult({
					status: 'error',
					message:
						'Se pudo escribir pero no leer los datos. Verifica los permisos.',
				});
			}
		} catch (error) {
