...
export default function FirebaseStatus() {
  ...
  const checkFirebaseStatus = async () => {
    ...
    try {
      ...
      if (isInitialized && config.projectId && typeof collection === "function" && typeof getDocs === "function") {
        try {
      ...
      setStatus({
        ...
        error: missingVars.length > 0 ? `Faltan variables de entorno: ${missingVars.join(", ")}` : null,
        config,
      })
    } catch (error) {
        ...
        <div className="text-sm">
          {status.checking ? (
            ...
            <div className="text-red-500 space-y-1">
              ...
              {status.error && <p className="text-xs bg-red-50 p-2 rounded">{status.error}</p>}
              <p className="text-xs text-gray-500 mt-2">
                La aplicación está funcionando en modo offline. Los datos se guardarán localmente.
              </p>
            </div>
          )}
          ...
          <div className="mt-2 pt-2 border-t text-xs text-gray-500">
            ...
            <p>API Key: {status.config.apiKey ? "Configurada" : "No configurada"}</p>
            <p>Auth Domain: {status.config.authDomain ? "Configurado" : "No configurado"}</p>
          </div>
        </div>
      </CardContent>
    </Card>
