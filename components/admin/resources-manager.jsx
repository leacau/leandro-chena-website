...
export default function ResourcesManager() {
		...
		try {
			...
			if (firestoreReady && db) {
				try {
					...
					setFirebaseError(
						firestoreError.message || 'Error al conectar con Firestore'
					);
					toast({
	...
	const saveToFirestore = async (resourceData, isUpdate = false) => {
		...
		while (retryCount < maxRetries && !success) {
			try {
				// Importar de nuevo para asegurarnos de tener las funciones más recientes
				const { collection, doc, addDoc, setDoc, updateDoc } = await import(
					'firebase/firestore'
				);
		...
		try {
			...
			if (success) {
				...
				toast({
					...
					description:
						'Los cambios se han guardado localmente y se sincronizarán más tarde.',
					variant: 'destructive',
				});
			...
			try {
				...
				if (firestoreReady && db) {
					...
					toast({
						title: 'Modo offline',
						description:
							'El recurso se ha eliminado localmente y se sincronizará cuando se restablezca la conexión.',
					});
				}
			} catch (error) {
	...
	const syncWithDatabase = async () => {
		if (!firestoreReady || !db) {
			toast({
				title: 'Error de sincronización',
				description:
					'Firestore no está inicializado correctamente. No se puede sincronizar.',
				variant: 'destructive',
			});
			return;
		}
		...
		try {
			...
			// Importar de nuevo para asegurarnos de tener las funciones más recientes
			const { collection, doc, getDocs, setDoc, writeBatch } = await import(
				'firebase/firestore'
			);
			...
			{firebaseError && (
				...
				>
					...
					<p className='text-sm mt-2'>
						La aplicación está funcionando en modo offline. Los cambios se
						guardarán localmente y se sincronizarán cuando se resuelva el
